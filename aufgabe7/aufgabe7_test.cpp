...
TRT test_hairpin()
{
  ...
  p1.reset();
  if (s == PDA::State::IN_PROGRESS) ++points;
  else
  {
    std::cerr << "PDA should return IN_PROGRESS, but did not.\n";
  }
  
  points /= 3; // one point for the three tests above
  ...
  p1.reset();
  for (auto e : string("gccg$"))
  {
    s = p1.next(e);
  }
  if (s == PDA::State::FAIL)  points += 2;
  else std::cerr << "'gccg$' should return FAIL, but did not.\n";
